    ...
    public class AIController : ControllerBase
    {
        ...
        public async Task<IActionResult> Chat([FromBody] ChatRequest request)
        {
            try
            {
                ...
                Console.WriteLine($"?? API Key starts with: {apiKey?.Substring(0, Math.Min(10, apiKey?.Length ?? 0))}...");
                
                if (string.IsNullOrEmpty(apiKey) || apiKey == "your-api-key-here")
                {
                    Console.WriteLine("? API key not configured!");
                    return BadRequest(new { error = "API key not configured. Please add your OpenAI API key to appsettings.json" });
                }
                ...
                Console.WriteLine($"?? Sending request to OpenAI with {request.Messages.Count} messages...");
                ...
                return StatusCode((int)response.StatusCode, new { error = "OpenAI API error", details = responseContent, statusCode = (int)response.StatusCode });
            }
            catch (Exception ex)
            {
